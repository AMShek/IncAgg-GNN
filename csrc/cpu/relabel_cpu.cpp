...
relabel_one_hop_cpu(torch::Tensor rowptr, torch::Tensor col,
  ...
  if (optional_value.has_value()) {
    ...
    AT_DISPATCH_ALL_TYPES(optional_value.value().scalar_type(), "relabel", [&] {
      ...
      offset = 0;
      // Iterate over each node in idx
      for (int64_t i = 0; i < idx.numel(); i++) {
        v = idx_data[i];
        row_start = rowptr_data[v], row_end = rowptr_data[v + 1];

        // Iterate over the edges of node v to find its neighbors
        for (int64_t j = row_start; j < row_end; j++) {
          w = col_data[j];
          it = n_id_map.find(w);
          // If w is not in the map, assign a new ID to w
          if (it == n_id_map.end()) {
          ...
          // If w is already in the map, use the existing ID
          else {
            out_col_data[offset] = it->second;
          }
          // Copy the edge value if it exists
...
relabel_one_hop_within_batch_cpu(torch::Tensor rowptr, torch::Tensor col, torch::optional<torch::Tensor> optional_value, torch::Tensor idx, bool bipartite) {
  ...
  out_rowptr_data[0] = 0;
  int64_t v, w, offset = 0;

  // Populate n_id_map with the nodes in idx
  for (int64_t i = 0; i < idx.numel(); i++) {
    v = idx_data[i];
    n_id_map[v] = i;
  }
  ...
  for (int64_t i = 0; i < idx.numel(); i++) {
    v = idx_data[i];
    int64_t row_start = rowptr_data[v], row_end = rowptr_data[v + 1];

    for (int64_t j = row_start; j < row_end; j++) {
      w = col_data[j];
      if (n_id_map.find(w) != n_id_map.end()) {
        offset++;
      }
    }
    out_rowptr_data[i + 1] = offset;
    ...
    AT_DISPATCH_ALL_TYPES(optional_value.value().scalar_type(), "relabel", [&] {
      ...
      for (int64_t i = 0; i < idx.numel(); i++) {
        ...
        for (int64_t j = row_start; j < row_end; j++) {
          w = col_data[j];
          it = n_id_map.find(w);
          // Only keep the edge if both ends are in idx
          if (it != n_id_map.end()) {
            out_col_data[offset] = it->second;
            // Copy the edge value if it exists
            out_value_data[offset] = value_data[j];
  ...
  if (!bipartite)
    out_rowptr = torch::cat(
        {out_rowptr, torch::full({(int64_t)n_id_map.size()}, out_col.numel(),
                                 rowptr.options())});
