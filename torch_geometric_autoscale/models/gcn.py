...
class GCN(ScalableGNN):
    ...
    def VR_forward(self, x: Tensor, adj_t: SparseTensor, drift_norm: int, epoch: int, batch_idx: int,  *args) -> Tensor:
        ...
        for i, (conv, bn) in enumerate(zip(self.convs[:-1], self.bns)):
            ...
            # Save 'x' and 'period_hist_selected_new' for epochs 295-299 and batch 0
            # if 297 <= epoch <= 299 and batch_idx == 0:
            #     torch.save(x, f'x_epoch_{epoch}_batch_0_layer_{i}.pt')
            #     torch.save(period_hist_selected_new, f'period_hist_selected_new_epoch_{epoch}_batch_0_layer_{i}.pt')
                # torch.save(adj_t, f'adj_t_epoch_{epoch}_batch_0_layer_{i}.pt')
            

            # ipdb.set_trace() # compare the two implementations
            ...
            ### vr update option 1
            # h = conv.lin(x)
            # h = adj_t @ h - conv.lin(adj_t @ period_hist_selected - period_hist_aggr_selected)  
            # h = adj_t @ h - conv.lin(adj_t @ period_hist_selected_new - period_hist_aggr_selected_new)
            # h = adj_t @ h # without VR term, for debugging
            # if conv.bias is not None:
            #     h = h + conv.bias
            ...
            h = conv.lin(h) # 使用VR得到的embeddings
            if conv.bias is not None:
                h = h + conv.bias
            ...
            ### history push and pull
            # x, time_push_and_pull = self.push_and_pull(hist, x, batch_size=args[0], n_id=args[1].to(hist.emb.device), offset=args[2] , count=args[3] ) #更新hist, original
            # time_data_movement += time_push_and_pull
            ...
            # torch.cuda.synchronize()
            # time_layer = start_time_layer.elapsed_time(end_time_layer) / 1000  # Convert milliseconds to seconds
            # log.info(f'Time taken for layer {i}: {time_layer} seconds')

        # time_before_last_layer = time.perf_counter() - t
        # log.info(f'Time taken before forwarding the last layer: {time_before_last_layer} seconds')
        ...
        # if 297 <= epoch <= 299 and batch_idx == 0:
        #     torch.save(x, f'x_epoch_{epoch}_batch_0_layer_{len(self.convs)-1}.pt')
        #     torch.save(period_hist_selected_new, f'period_hist_selected_new_epoch_{epoch}_batch_0_layer_{len(self.convs)-1}.pt')
        #     torch.save(adj_t, f'adj_t_epoch_{epoch}_batch_0_layer.pt')
        ...
        """
        ### vr update option 1: 先lin()再@
        h = self.convs[-1].lin(x)
        # h = adj_t @ h - self.convs[-1].lin(adj_t @ period_hist_selected - period_hist_aggr_selected)
        h = adj_t @ h - self.convs[-1].lin(adj_t @ period_hist_selected_new - period_hist_aggr_selected_new)
        # h = adj_t @ h # without VR term, for debugging
        h = h + self.convs[-1].bias 
        """
        ...
        # torch.cuda.synchronize()
        # time_layer = start_time_layer.elapsed_time(end_time_layer) / 1000  # Convert milliseconds to seconds
        # log.info(f'Time taken for the last layer: {time_layer} seconds')


        # time_before_last_layer = time.perf_counter() - t
        # log.info(f'Time taken after forwarding the last layer: {time_before_last_layer} seconds')
